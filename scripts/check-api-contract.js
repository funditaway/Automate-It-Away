...
async function main() {
 ...
 const deskStatusSrc = healthSrc.split("async function deskStatus")[1] || "";
 const deskStatusBody = deskStatusSrc.split("async function health")[0] || "";
 if (!/automation\s*:\s*\{[\s\S]*deskAi/.test(deskStatusBody)) fail("status.js deskAi pub");
 else pass("h.st.desk");
 ...
 const stDesk = ((st.body.automation || {}).deskAi || {});
 if (!(st.body.automation || {}).deskAi) fail("status deskAi");
 else if (typeof stDesk.on !== "boolean") fail("status deskAi.on");
 else if (!stDesk.note) fail("status deskAi.note");
 else if ((st.body.automation || {}).grok && !(st.body.automation || {}).deskAi) fail("status need deskAi");
 else if (stDesk.on && stDesk.note !== "A Desk AI can draft.") fail("status deskAi on note");
 else if (!stDesk.on && stDesk.note !== "A Desk AI can't draft on this phone yet.") fail("status deskAi off note");
 else pass("st.deskAi");
