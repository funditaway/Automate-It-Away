...
if (yesNo.indexOf("Drop widget `#drop-on` leftover after that pass") < 0) fail("ACCOUNT-YES-NO must name Drop widget #drop-on leftover");
else pass("ACCOUNT-YES-NO names Drop widget #drop-on leftover");
if (packMd.indexOf("Drop widget `#drop-on` leftover:") < 0) fail("PACK.md must name Drop widget #drop-on leftover");
else pass("PACK.md names Drop widget #drop-on leftover");
if (yesNo.indexOf("Drop `#drop-on` keep leftover after that pass") < 0) fail("ACCOUNT-YES-NO must name Drop #drop-on keep leftover");
...
else pass("DROP.md says /drop paints #drop-on at step one");
if (dropMd.indexOf("still paints `#drop-on` at step one") < 0) fail("DROP.md must keep #drop-on painted at /drop step one");
else pass("DROP.md keeps #drop-on painted at /drop step one");
if (dropMd.indexOf("it hushes the `#drop-on` banner") >= 0) fail("DROP.md must not say step one hushes #drop-on");
else pass("DROP.md does not hush #drop-on on /drop");
const nowSrc = fs.readFileSync(path.join(root, "drop-now.js"), "utf8");
if (nowSrc.indexOf("function widgetOn") < 0) fail("drop-now.js must detect the /widget path");
else pass("drop-now.js detects the /widget path");
if (nowSrc.indexOf("function slimChrome") < 0) fail("drop-now.js must slim #drop-on on /widget");
else pass("drop-now.js slims #drop-on on /widget");
...
else pass("drop-steps.js paint() keeps #drop-on visible on /drop");
...
else pass("PACK.md names Drop widget pick href leftover");
if (dropMd.indexOf("stays on `/widget?ws=`") < 0) fail("DROP.md must say /widget world pick stays on /widget?ws=");
else pass("DROP.md says /widget world pick stays on /widget?ws=");
...
} else pass("DROP.md says /widget pick does not dump onto the rail");
const pickSrc = fs.readFileSync(path.join(root, "drop-pick.js"), "utf8");
if (pickSrc.indexOf("function widgetOn") < 0) fail("drop-pick.js must detect the /widget path");
else pass("drop-pick.js detects the /widget path");
if (pickSrc.indexOf("function dropHref") < 0) fail("drop-pick.js must pick /widget vs /drop");
else pass("drop-pick.js picks /widget vs /drop");
if (pickSrc.indexOf("location.href = dropHref(use)") < 0) fail("drop-pick.js goDrop must use dropHref");
