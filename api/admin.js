...
function personHint(src) {
  ...
  const rawPending = String(hint.includePending != null ? hint.includePending : (hint.pending || "")).toLowerCase();
  return {
    ...
    includePending: hint.includePending === true || rawPending === "1" || rawPending === "true" || rawPending === "yes"
  };
}
function seatMatchesPerson(seat, hint) {
  if (!seat) return false;
  if (hint.id && seat.id === hint.id) return true;
  if (seat.status === "pending" && !hint.includePending) return false;
...
function seatThread(seat) {
  return (Array.isArray(seat && seat.thread) ? seat.thread : []).slice(-20);
}
function addSeatLine(seat, line) {
  if (!seat) return;
  seat.thread = Array.isArray(seat.thread) ? seat.thread : [];
  seat.thread.push(line);
  if (seat.thread.length > 80) seat.thread = seat.thread.slice(-80);
}
function findSeat(row, body) {
  if (!row) return { ok: false, status: 404, error: "No desk." };
  const id = String((body && (body.id || body.personId)) || "").trim();
  if (!id) return { ok: false, status: 400, error: "Person id required." };
  const seat = (row.people || []).find((p) => p && p.id === id);
  if (!seat) return { ok: false, status: 404, error: "Person not found." };
  return { ok: true, seat };
}
function sayOnSeat(row, body, actor) {
  const hit = findSeat(row, body);
  if (!hit.ok) return hit;
  const seat = hit.seat;
  if (!isOwner(actor) && (!actor || actor.id !== seat.id)) return { ok: false, status: 403, error: "Owner or matching person required." };
  const text = String((body && body.text) || "").trim().slice(0, 500);
  if (!text) return { ok: false, status: 400, error: "Text required." };
  addSeatLine(seat, {
    from: (actor && actor.name) || "desk",
    kind: String((body && body.kind) || "say"),
    text,
    t: new Date().toISOString(),
    side: isOwner(actor) ? "owner" : "person"
  });
  return { ok: true, status: 200, person: publicPerson(seat), thread: seatThread(seat) };
}
...
function personBook(hint, opts) {
  ...
  const thread = seats.reduce((out, seat) => out.concat(seatThread(seat)), []).slice(-20);
  const pending = seats.some((seat) => seat && seat.status === "pending");
  return {
    ok: true,
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (q.who || q.person || q.name) {
      const book = personBook({
        ...
        includePending: q.includePending || q.pending || ""
  ...
  if (action === "request") {
    ...
    if (seat) addSeatLine(seat, { from: seat.name || "request", kind: "request", text: "Requested a seat.", t: new Date().toISOString(), side: "person" });
    await save();
    return res.status(202).json({ ok: true, pending: true, person: made.person });
  }
  if (action === "say" || action === "talk") {
    const made = sayOnSeat(row, body, person);
    if (!made.ok) return res.status(made.status || 400).json({ ok: false, error: made.error });
    await save();
  ...
  if (action === "approve" || action === "deny") {
    ...
    if (!made.ok) return res.status(made.status || 400).json({ ok: false, error: made.error });
    const seat = (row.people || []).find((p) => p && made.person && p.id === made.person.id);
    if (seat) addSeatLine(seat, { from: person.name || "owner", kind: action, text: action === "deny" ? "Denied this request." : "Approved this request.", t: new Date().toISOString(), side: "owner" });
    await save();
  ...
  return res.status(400).json({ ok: false, error: "Unknown admin action.", actions: ["invite", "request", "say", "ask", "permit", "mine", "person", "approve", "deny", "plan", "login"] });
};
