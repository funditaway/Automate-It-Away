...
function draftConsign(job) {
  ...
  return title + ". " + cond + "Ask " + band + ". Draft only — nothing listed until Yes.";
}
...
function draftHome(job) {
  ...
  if (/form|permission|slip|school/.test(blob(job))) {
    return "Form note: " + title + "." + who + when + " Sign and send only after you tap Yes.";
  }
  if (/oil change|grocery|lawn|chore|pickup|practice|dentist|doctor/.test(blob(job))) {
    return "Reminder: " + title + "." + who + when + pay + " Nothing goes to anyone until Yes.";
  }
  return title + "." + who + when + pay + " Draft only — Yes or Stop.";
}

function draftGeneric(job) {
  const title = job.title && job.title !== "Untitled" ? job.title : "Job from the desk";
  const extras = job.custom
    ? Object.keys(job.custom).filter((k) => job.custom[k]).map((k) => k + " " + job.custom[k]).join(". ")
    : "";
  return title + (job.notes ? " · " + job.notes : "") + (extras ? " · " + extras : "") + ". Draft only — Yes or Stop.";
}
...
function qualifyHome(job, text, missing) {
  ...
  let next;
  if (risk === "legal") {
    why = "This would name a child or school in public. Ask me before Yes.";
    ...
    next = "Owner or helper taps Yes if it still fits.";
  } else if (missing.length) {
    why = "Need " + missing.join(" and ") + " before this is ready.";
    next = "Add the missing bit, then Yes or Stop.";
  } else {
    why = "House draft is ready. Tap Yes or Stop.";
    next = "Yes sends the reminder or form note. Stop if it is already done.";
  }
...
function qualifyJob(job, shopOrModel) {
  ...
  if (pack === "home") {
    ...
    if (risk === "title") {
      ...
      next = "Owner checks title, then Yes or Stop.";
    } else if (risk === "price") {
      why = "Ask sits outside the comps band. Ask me before Yes.";
      next = "Owner sets the ask, then Yes or Stop.";
    } else if (missing.length) {
      why = "Need " + missing.join(" and ") + " before this is ready.";
      next = "Add the missing bit, then Yes or Stop.";
    } else {
      why = "Listing draft is ready. Tap Yes or Stop.";
    }
  } else if (pack === "vita") {
    ...
    why = missing.length
      ...
      : "Draft is ready. Tap Yes or Stop.";
  }

  const amount = moneyOf(job);
  const rules = shop ? ensureRules(shop) : [];
  const holdAt = shop ? moneyWaitOf(rules) : null;
  const stopWhy = ruleWhy(rules, job, "qualify") || ruleWhy(rules, job, "do");
  if (ruleWantsStop(rules, job, "qualify") || ruleWantsStop(rules, job, "do")) {
    why = stopWhy || "A desk rule says stop. Owner taps.";
    next = "Stop until the owner taps.";
  } else if (ruleWantsOwner(rules, job, "qualify") || ruleWantsOwner(rules, job, "do") || moneyNeedsOwner(amount, holdAt)) {
    why = (stopWhy || why ? (stopWhy || why) + " " : "") + "Waiting on the owner.";
  ...
  const rail = risk === "legal" || risk === "title"
    || moneyNeedsOwner(amount, holdAt)
    || ruleWantsOwner(rules, job, "qualify")
    || ruleWantsOwner(rules, job, "do")
    || ruleWantsStop(rules, job, "qualify")
    || ruleWantsStop(rules, job, "do");
...
function crewOf(job, shop) {
  ...
  if (job.assignee) {
    return { id: "human", label: job.assignee, does: "A person on this desk has it." };
  }
  if (job.draft) {
    return { id: "doer", label: "Doer", does: "Draft only. You tap Yes or No." };
  }
...
function recommend(job, missing, shop) {
  const recs = [];
  if (missing && missing.length) recs.push({ kind: "ask", text: "Need " + missing.join(" and ") + " before Yes." });
  ...
  if (job.pack === "home") {
    ...
    recs.push({ kind: "next", text: "Phone calendar file is ready when you tap Yes. Google stays off until the key is on the box." });
  }
  ...
  matchingRules(rules, job, job.step === "Collect" ? "collect" : job.step === "Follow" ? "follow" : "qualify")
    .concat(matchingRules(rules, job, "do"))
    .forEach((r) => {
      if (r && r.text && !recs.some((x) => x.text === r.text)) recs.push({ kind: "rule", text: r.text });
    });
  if (!recs.length) recs.push({ kind: "next", text: "Draft is ready. Tap Yes or Stop." });
  job.crew = crewOf(job, shop);
  if (job.crew && job.crew.label) recs.push({ kind: "crew", text: "AIA next · " + job.crew.label + " — " + job.crew.does });
  job.recs = recs.slice(0, 8);
  job.promptVersion = job.promptVersion || "desk-rules-3";
  return job;
}
...
function markFlow(job, step) {
  ...
  const last = job.flow[job.flow.length - 1];
  if (last && last.step === step) return job;
  job.flow.push({ step: String(step || "").slice(0, 24), at: new Date().toISOString() });
  job.flow = job.flow.slice(-12);
  return job;
}
...
module.exports = { qualifyJob, followJob, runWorkspace, detectPack, recommend, icsOf, whenOf, markFlow, crewOf, MONEY_HOLD, PACKS };
