...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "carry") {
      ...
      const waitHit = moneyNeedsOwner(amount, holdAt)
        || ruleWantsOwner(rules, job, "do")
        || ruleWantsStop(rules, job, "do")
        || ruleWantsOwner(rules, job, "follow")
        || ruleWantsStop(rules, job, "follow");
      if (waitHit && shop && !isOwner(person)) {
        return res.status(403).json({
          ok: false,
          error: ruleWhy(rules, job, "do") || ruleWhy(rules, job, "follow") || "Waiting on the owner to mark this done.",
          job
        });
      }
    ...
    return res.status(400).json({ error: "action must be capture, qualify, recommend, ship, kill, say, ask, fill, define-field, assign, or carry" });
  }
