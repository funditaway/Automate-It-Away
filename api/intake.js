...
function verdict(answers) {
  ...
  const ready = [];
  const held = [];
  if (/bind|carrier|illustration|policy issue/.test(blob)) blocked.push("We do not bind coverage or send a carrier app from this desk.");
  if (/no one tap|fully automatic money|send money without me|auto pay over/.test(blob)) blocked.push("Money wait stays an owner rule. That does not get automated away.");
  if (need.includes("whatnot")) blocked.push("Whatnot stays off.");
  const known = /photo|consign|resale|list|ebay|payout|call|text|book|calendar|form|widget|website|queue|invoice|pickup|home|family|school|oil change|grocery|chore|reminder|house list/.test(blob);
  need.forEach((id) => {
...
function publicIntake(row) {
  return { id: row.id, step: row.step, answers: row.answers, messages: row.messages, verdict: row.verdict || null, requestId: row.requestId || null, jobId: row.jobId || null, who: row.who || null, next: row.next || null };
}
function titleOf(text) {
  const clean = String(text || "").trim().replace(/\s+/g, " ");
  return (clean.split(/[.!?]/)[0] || clean).slice(0, 80) || "Desk note";
}
...
module.exports = async function handler(req, res) {
  ...
  if (action === "do") {
    ...
    if (kind === "draft" && job) {
      qualifyJob(job, shop);
      row.who = whoFor(job, shop, person);
      row.messages.push({ from: "desk", text: replyFor(job, row.who) });
  ...
  if (action === "request") {
    const ticket = { id: "t_" + Date.now().toString(36), workspace, title: "Automation request", body: Object.values(row.answers).join("\n"), from: body.name || "shop", email: body.email || "", role: "request", status: "open", createdAt: new Date().toISOString() };
    mem.tickets.unshift(ticket);
    row.requestId = ticket.id;
    row.messages.push({ from: "desk", text: "Request sent to Automate It Away. Ticket " + ticket.id + ". We will not pretend it is live." });
