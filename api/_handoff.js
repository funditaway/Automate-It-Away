...
function isApprovedAgent(person) {
  ...
  return (kind === "agent" || !!person.deskAi) && person.status === "approved";
}
function agentSpec(who) {
  if (!who) return null;
  const named = who.deskAi
    ? {
...
function crewOf(job, shop) {
  ...
  if (job.assignee) {
    ...
    if (isApprovedAgent(who)) {
      ...
      return { id: String(spec.crew).toLowerCase(), label: who.name || spec.crew, kind: "agent", does: spec.does, artifact: spec.artifact, deskAi: !!who.deskAi };
    }
...
function applyHandoff(job, who, shop) {
  ...
  if (isApprovedAgent(who)) {
    job.waitingOn = "agent";
    job.next = (who.name || who.crew) + " drafts on the card. A person taps Yes or Stop.";
...
function agentDraft(job, who) {
  ...
  const namedLine = who.deskAi
    ? ((who.prompt && String(who.prompt).trim()) || (who.name + " draft for this desk: " + title + ". " + (notes || spec.does) + " Draft only — a person taps Yes or Stop."))
    : "";
  const bits = {
