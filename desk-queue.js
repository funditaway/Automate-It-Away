...
(function () {
  ...
  window.helpWithAi = async function (id) {
    ...
    const line = out.status >= 400 ? ((out.data && out.data.error) || "Could not draft help.") : "A Desk AI drafted on the card. Nothing sent.";
  ...
  window.queueCard = function (j, staff) {
    ...
    const line = outDesk ? (next || "Off the desk. Confirm done, or tap Needs a hand.") : decide ? (next || "Yes or Stop.") : (next || "Hand off, send off the desk, or ask the desk.");
