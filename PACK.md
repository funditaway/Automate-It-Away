...
Status public dump leftover: after Status chrome used Desk AI voice, `/status` below-fold still dumped raw `/api/health` + `/api/status` JSON that named blob and Decentraweb. Those strings are gone from public payloads. Persist fingerprint is store.save. Below-fold dump no longer paints raw health JSON. Did not redo Open this card. Did not redo #228 chrome copy. Collect HOLD. No silent send.
