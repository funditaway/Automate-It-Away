...
Drop widget empty Tell leftover: after `/widget` skipped This drop, embed `/widget` still owned Tell (preview hookType; chat skips embed). Empty Tell had no skip — two empty taps painted two Type the work. A Desk AI drafts the card. lines. Embed empty Tell now skips a second blank-chat prompt, same as chat on `/drop`. Card preview + one Tell the desk thread stay. Full Drop chrome stay later. Seat id `agent` stays. Collect HOLD. No silent send.
...
Drop desk-first steps leftover: after embed `/widget` skipped the Talk bar, `/drop` still stacked every card in one column and buried **Which desk gets this** under the world search and the modes. Drop now runs one step at a time — Desk · Tell · Card · Check · Share (`drop-steps.js`). Desk is step one, the world search seats under it, a sticky rail plus Back / Next walks the rest, and off-step cards are `.step-off`. Picking a desk moves the rail to Tell. The rail always carries Draft only. You still tap Yes or Stop. Nobody sends money from here. Embed skips the rail. No card copy changed. Seat id `agent` stays. Collect HOLD. No silent send.

Drop widget steps rail leftover: after `/widget` skipped Drops from this phone, drop-steps.js still painted Desk · Tell · Card · Check · Share on `/widget`. `onDrop()` matches widget.html and only embed skipped the rail. Those faces now skip the step rail. `/drop` still paints the rail. Embed still skips. Header / modes / share stay on `/drop`. Seat id `agent` stays. Collect HOLD. No silent send.
...
Drop widget standalone steps rail leftover: after `#drop-on` stay painted on `/drop`, Counter phone standalone `/widget` still showed `#drop-steps` display:block. Source already had `boot()` `widgetOn()` early return and widgetHref → `/widget`; no static `#drop-steps` in markup. Earlier prod PASS only checked `#drop-on`. Those faces now do not load `drop-steps.js` (`skipSteps()`). `drop-now.js` `slimChrome()` tears down `#drop-steps` if a leftover rail painted. Embed still 0. `/drop` still paints the rail and `#drop-on` at Desk step. Seat id `agent` stays. Collect HOLD. No silent send.
...
Drop widget standalone Talk bar leftover: after world pick stayed on `/widget?ws=`, drop-talk.js still unhid `#talkBar` (Hear this / Talk / Quiet) on standalone `/widget`. Preview already skipped Talk on embed. World `/widget` then showed the Talk bar next to Tell. Those faces now skip Talk, same as embed. `/drop` still paints Talk. Preview still owns Tell on `/widget`. Card preview + one Tell the desk thread stay. Seat id `agent` stays. Collect HOLD. No silent send.
...
History drafted-card trail leftover: after Create Open/Unlock clear was honest, `/history` still offered Clear filters on a truly empty trail and never labeled Create / Drop drafted Yes / Stop cards. Empty History now says the trail is empty — Clear filters (empty box and the toolbar tap) only when a filter hid the trail. Create / Drop drafted cards label Create / Drop and Yes / Stop / Kill when they still wait on a human. Pack Install / Give / Update stay off until a desk is open. Collect HOLD. No silent send.
...
Status raw health dump leftover: after Counter chrome used Desk AI voice, `/status` below the fold still dumped raw `/api/health` + `/api/status` JSON that named blob and Decentraweb. That dump is gone from the phone. Saved work / Photos and files / Name register stay plain. Machine health JSON for scripts is unchanged. Did not redo History / Create / Queue. Collect HOLD. No silent send.

History drafts-off leftover: after Create / More / Status said A Desk AI can't draft on this phone yet, `/history` still said XAI_API_KEY / this box and dumped a Grok crew note on drafts-on. History is a desk tab. Checking / on / off / fail now use Desk AI voice. Pack fail says Could not reach the desk. Did not redo History trail. Did not redo Create / More / Status. Queue Pipes sheet and Status store row stay. Collect HOLD. No silent send.
...
Field tips + Ask AIA: `aia-tip.js` on How, Setup, Home, onboard, login, Drop, widget, Account, Examples, Help, Support, Pipes, Connections, Create, and Studio. Extra info on the field. Ask AIA opens Help chat (`/support`) with field id + plain tip text + page. Quiet on Help chat stops speech and keeps that answer — it does not wipe back to the stock prompt. Help chat stays draft / help. Need a person? Drop a card on the AIA Admin desk. Yes / Stop stay human. Give pack / Update pack named CTAs on world doors, Help, and Account. A pack puts When → If → Then on this desk queue; buyer binds their own keys and devices; Yes / Stop / Kill before outbound or hardware; App / webhook is the live pipe. Drop / widget also tip I am, kind, phone, photo, Put data on paste / tell, and injected kind-fields What is needed / When / Preferred outcome / Where / Who it is for / From / Amount note / Callback number — Drop writes a card; Yes before anything leaves; Collect HOLD; no silent send. Thin App / webhook pack example on Help `#build-pack`, Examples, and Studio — copy, do not seed; not a listed SKU; not a bindings product. consign.html Drop it / Yes. Not a ticket portal. Not a new chat product. No ETH escrow, micro-wei, streaming ETH, or L2 state channels.
