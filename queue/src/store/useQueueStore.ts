...
export const useQueueStore = create<QueueStore>((set, get) => ({
  ...
  pendingCards: () =>
    sortQueue(get().cards.filter((c) => c.status === 'pending' || get().exitingCardIds.includes(c.cardId))),
  ...
  signCard: (cardId) => {
    ...
    const pending = sortQueue(
      get().cards.filter(
        (c) => c.cardId !== cardId && c.status === 'pending' && !get().exitingCardIds.includes(c.cardId),
      ),
    )
