    ...
    function fields() {
      ...
      if (kind === "ai") return `<label>Name this desk AI</label><input name="name" required placeholder="James’s AI"><label>Role</label><select name="role"><option>Doer</option><option>Worker</option><option>Rail</option><option>Packer</option><option>Mapper</option></select><label>What it drafts</label><input name="does" placeholder="Drafts desk work for this project"><label>Steps it may draft</label><input name="steps" placeholder="qualify, do, follow"><label>Draft line</label><textarea name="prompt" rows="2" placeholder="Do not send. Do not invent money. Wait on Yes."></textarea><p class="hint">Bound to this desk. Never Yes, Stop, money, or mail. Collect stays HOLD. Pack it in Studio to list or keep private.</p>`;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        if (kind === "ai") {
          ...
          const r = await fetch("/api/desks", { method: "POST", headers: headers(), body: JSON.stringify({ action: "save-ai", name: name, role: f.get("role") || "Doer", does: f.get("does") || "", prompt: f.get("prompt") || "", steps: f.get("steps") || "qualify, do, follow" }) });
          const data = await r.json().catch(() => ({}));
