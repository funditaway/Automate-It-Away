...
- `drop-now.js` — Quick / recent / after-drop. `/widget`, embed, and `?embed=1` skip the `#drop-on` banner (This drop goes to … Change desk) and tear down `#drop-steps` if a leftover rail painted. `/drop` still paints `#drop-on` at step one (This drop goes to … Change desk) — not `display:none`. Change desk on that banner still points at `/drop`. Full Drop chrome (header / modes / share) stay on `/drop`.
