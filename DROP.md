...
- `drop-now.js` — Quick / recent / after-drop. `/widget`, embed, and `?embed=1` skip the `#drop-on` banner (This drop goes to … Change desk), keep `#talkBar` hidden, and tear down `#drop-steps` if a leftover rail painted. `/drop` still paints `#drop-on` at step one (This drop goes to … Change desk) — not `display:none`. `/drop` still paints Talk. Change desk on that banner still points at `/drop`. Recent public desks on `/widget` stay on `/widget?ws=`. `/drop` still opens `/drop?ws=`. Share links stay `/drop?ws=`. Full Drop chrome (header / modes / share) stay on `/drop`.
