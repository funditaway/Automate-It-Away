...
- Desks book leftover after blob 403 still: preview `b69995a` still `driver tmp-file`, `read=error` `status=403` `write=null` with `auth=oidc`. OIDC is selected but SDK `head`/`get`/`put` still 403 on this store (wrong storeId override, CDN pathname, or no blob scope), and hydrate never `put`s. Prefer `BLOB_READ_WRITE_TOKEN` / `BLOB_STORE_ID`. Let the SDK resolve auth with no forced token. If the SDK 403s, persist the same way `/api/upload` already does: sealed PUT/GET `https://blob.vercel-storage.com/aia/store.json`. Probe after onboard: `driver blob`, `read ok`, `write ok`. Wrong or empty pin still 401. Collect HOLD. `scripts/check-blob-store.js` covers SDK-403 → REST replica mine.
