...
- World-home field tips (`aia-tip.js` on `index.html`, `how.html`, `setup.html`, `onboard.html`): tap `?` for extra info. Ask AIA opens Help chat (`/support`) with the field answer — same Talk-to-AIA card door, not a new product. Help / tickets / messages move as cards between a world-user desk and AIA Admin. Not Zendesk. Agents / bots / guardrails / job cards stay this desk. Collect HOLD. No silent send. No future-Pack SKU, pack-partners market, ETH collect, AIA coin, or `drop-pack.js`.
- Help chat Quiet stops speech and restores the Ask AIA answer (or the stock prompt when there is no deep link). Hear this beside Quiet must not wipe the painted field answer.
- `scripts/check-aia-tip.js` covers that tooltip + Ask AIA honesty.
