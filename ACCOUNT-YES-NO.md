...
- Desks book leftover after blob 403 still: onboard 201 still left `/api/health` tmp-file. Live `ea9e298` REST GET works (`read empty` `auth rest` `access public`) but PUT failed `write fail` detail `[object Object]`. REST put now sends `x-vercel-blob-access: public` (overwrite retry), stringifies API error objects, and uses `aia-store.json` plus `aia/store.json`. Sealed body. Probe: `driver blob`, `read ok`, `write ok`, `auth rest` or oidc. Wrong pin 401. Collect HOLD.
