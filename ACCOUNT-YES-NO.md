...
- Desks book leftover after blob 403 still: preview `a0d101d` still `driver tmp-file`, `read=error` `status=403` `write=null` `auth=token` after onboard. `VERCEL_OIDC_TOKEN` is not in the Lambda env, so `_lib` still passed the static token and overrode SDK OIDC from the request-context header. Do not pass `token` when `BLOB_STORE_ID` is set. `head()` stays the empty check. `aia/store.json` is `put` private; a Public store fallback is AES-GCM sealed (not world-readable desks/PINs). Probe after onboard: `driver blob`, `read ok`, `access` private or public. Wrong or empty pin still 401. Collect HOLD. `scripts/check-blob-store.js` covers no-env OIDC + sealed public replica mine.
