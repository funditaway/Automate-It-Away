...
- Desks book leftover after blob 403 still: onboard 201 still left `/api/health` tmp-file. REST GET empty works; REST PUT failed `[object Object]`, then `Store is suspended` (preview) / `Cannot get store id from token or header` (www). REST put now matches the SDK: Buffer body, `x-api-version: 7`, `x-vercel-blob-store-id`, and `PUT https://vercel.com/api/blob/?pathname=` before the storage-host fallback. `blobErrText` never reports `[object Object]`. Health fingerprint `blob.stamp=put-v3` + `blob.rev=VERCEL_GIT_COMMIT_SHA`. Sealed body. Probe: `driver blob`, `read ok`, `write ok`. Wrong pin 401. Collect HOLD.
