...
- Desks book leftover after blob 403 still: preview still `driver tmp-file`, `read=error` `status=403` `access=null` `write=null` after onboard. SDK `get(pathname)` builds `{tokenStoreId}.{access}.blob.vercel-storage.com` and an explicit static token overrides OIDC, so CDN 403’d on both private and public; the read-error guard never `put`. Read now uses API `head()` (404 = empty, allow write), auth is OIDC+`storeId` when `VERCEL_OIDC_TOKEN` is present (no token override; token retry only if OIDC 403s), write verifies via the put/head URL, and onboard `save()` may `put` local desks even if CDN get 403’d (empty health still will not wipe). Probe after onboard: `driver blob`, `read ok`. Wrong or empty pin still 401. Collect HOLD. `scripts/check-blob-store.js` covers CDN-get 403 + head empty → replica mine.
