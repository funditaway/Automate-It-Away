...
- Desks book leftover after blob 403 still: preview after onboard 201 still `driver tmp-file`, `workspaces:0`, `write=null`, `read=error`. `b69995a` was SDK 403; `dfc94ef` health detail is `No blob credentials found` because the first SDK call used `{}` and that error is not a 403, so token/REST never ran on `/api/health`. Retry on credentials-missing as well as 403; pass `storeId` then explicit `BLOB_READ_WRITE_TOKEN*` ; REST sealed PUT/GET like upload. Probe: `driver blob`, `read ok`, `write ok`. Wrong pin 401. Collect HOLD.
